        ...
        <div class="flex flex-col gap-6">
            ...
            @if($filters['tab'] === 'evaluations')
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 text-sm text-slate-500">Le tableau des évaluations se charge ici dynamiquement.</div>
                </div>
            @endif
            ...
            @if($filters['tab'] === 'agents')
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <form method="GET" action="{{ route('rh.dashboard') }}" class="flex flex-wrap items-end gap-4">
                        ...
                        <div class="flex-1 min-w-[240px]">
                            <label class="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1.5 block">Recherche Agent</label>
                            <input type="text" name="search" value="{{ $filters['search'] }}" placeholder="Nom, prénom, matricule..."
                                class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold text-slate-700 outline-none focus:border-violet-500 focus:bg-white transition">
                        </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-slate-100">
                        <h3 class="font-black text-slate-800">Répertoire des collaborateurs</h3>
                    </div>
                    <div class="p-6 text-sm text-slate-500">La liste des agents est disponible avec la pagination.</div>
                </div>
            @endif
