...
@php
    ...
    $resolvedService   = $services->firstWhere('id', $resolvedServiceId) ?? $selectedService;
    $lockService       = $services->count() === 1 || $selectedService !== null;
@endphp
        ...
        <div class="w-full flex-col gap-6">
            ...
            <header class="admin-panel px-6 py-6 lg:px-8">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Espace Directeur / Ma direction</p>
            ...
            <section class="admin-panel px-6 py-6 lg:px-8">
                ...
                <form method="POST" action="{{ route('directeur.evaluations.store') }}" class="space-y-8">
                    @csrf
                        ...
                        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <input id="identification_matricule" name="identification[matricule]" type="text"
                                       value="{{ old('identification.matricule') }}" class="ent-input bg-slate-50 text-slate-600" readonly placeholder="Renseigné automatiquement">
                            </div>
                            <div class="space-y-2">
                        ...
                        <div class="grid gap-6 xl:grid-cols-2">
                                    ...
                                    <table class="min-w-full text-sm text-slate-700">
                                        <thead class="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
                                            <tr>
                                                ...
                                                <th class="px-3 py-3 text-left">Formation, diplômes ou autres titres</th>
                            ...
                            <div class="space-y-3">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-slate-500">Renseignez les principales expériences du parcours.</p>
                                </div>
                                <div class="overflow-x-auto rounded-2xl border border-slate-200">
                                    <table class="min-w-full text-sm text-slate-700">
                                        <thead class="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
                    ...
                    <section id="objective-section" class="space-y-5 border-t border-slate-200 pt-8">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-slate-500">Choisissez une fiche d'objectifs, puis renseignez les sous-critères et leurs notes. Barème : 1 à 5.</p>
                            </div>
                    ...
                    <section id="subjective-section" class="space-y-5 border-t border-slate-200 pt-8">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-slate-500">Renseignez les sous-critères et leurs notes. Barème : 1 à 5.</p>
                            </div>
                    ...
                    <section class="space-y-5 border-t border-slate-200 pt-8">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500">Cet espace conserve les moyennes pondérées, les notes par critères et la note totale.</p>
                        </div>
                    ...
                    <section class="space-y-5 border-t border-slate-200 pt-8">
                        ...
                        <div class="grid gap-5 md:grid-cols-2">
                            <div class="space-y-2">
                                <label for="signature_evalue_nom" class="text-xs font-semibold uppercase tracking-[0.15em] text-slate-500">Évalué (Chef de service)</label>
                            ...
                            <div class="space-y-2">
                                <label for="signature_evaluateur_nom" class="text-xs font-semibold uppercase tracking-[0.15em] text-slate-500">Évaluateur (Directeur)</label>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function renderObjectiveChoices() {
            ...
            objectiveChoiceContainer.innerHTML = `
                <div class="space-y-3">
                    <p class="text-sm font-semibold text-slate-800">Objectifs disponibles dans la fiche sélectionnée</p>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (selService) {
            selService.addEventListener('change', function () {
                ...
                fillIdent(data);
                if (data?.chef_agent_id && window.sgpFillFormations) window.sgpFillFormations(data.chef_agent_id);
            });
        }
        if (selAgence) {
        ...
        if (selCaisse) {
            selCaisse.addEventListener('change', function () {
                ...
                fillIdent(data);
                if (data?.directeur_agent_id && window.sgpFillFormations) window.sgpFillFormations(data.directeur_agent_id);
            });
        }
    });
    </script>
