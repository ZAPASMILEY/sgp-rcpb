    ...
    <div class="w-full flex flex-col gap-6">
        ...
        <header class="admin-panel px-6 py-6 lg:px-8">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Espace Directeur / Subordonnés / {{ $service->nom }}</p>
                    ...
                    @if ($chef)
                        <p class="mt-2 text-sm text-slate-600">Chef de service : <span class="font-semibold">{{ $chef }}</span></p>
                    @endif
                </div>
            ...
            @if ($tab === 'evaluations')
                ...
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center text-sm text-slate-400">
                        Aucune évaluation créée pour ce service.
                    </div>
                @endforelse
                    ...
                    <div class="mb-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <div class="flex items-center gap-2">
                                <span class="inline-flex rounded-full border px-3 py-1 text-[11px] font-black {{ $sc }}">{{ $sl }}</span>
                                <a href="{{ route('directeur.subordonnes.service.objectifs.show', $fiche) }}" class="ent-btn ent-btn-soft text-xs">Voir</a>
                            </div>
                        </div>
                    </div>
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center text-sm text-slate-400">
                        Aucune fiche d'objectifs assignée à ce service.
                    </div>
                @endforelse
            @endif
        </div>
