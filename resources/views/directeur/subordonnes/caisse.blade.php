    ...
    <div class="w-full flex flex-col gap-6">
        ...
        <header class="admin-panel px-6 py-6 lg:px-8">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Espace Directeur / Subordonnés / {{ $caisse->nom }}</p>
                    <h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{{ $caisse->nom }}</h1>
                    @if ($directeurUser)
                        <p class="mt-2 text-sm text-slate-600">
                            Directeur de caisse : <span class="font-semibold">{{ $directeurUser->name }}</span>
                        </p>
                    @endif
                    @if ($caisse->ville)
                        <p class="mt-1 text-xs text-slate-400">{{ $caisse->ville->nom ?? '' }}</p>
                    @endif
                </div>
        ...
        <div class="admin-panel px-6 py-6 lg:px-8">
            ...
            @if ($tab === 'evaluations')
                ...
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center text-sm text-slate-400">
                        Aucune évaluation créée pour cette caisse.
                    </div>
                @endforelse
            ...
            @else
                ...
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center text-sm text-slate-400">
                        Aucune fiche d'objectifs assignée à cette caisse.
                    </div>
                @endforelse
            @endif
        </div>
