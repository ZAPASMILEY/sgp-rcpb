    ...
    <div class="w-full flex flex-col gap-6">
        ...
        <header class="admin-panel px-6 py-6 lg:px-8">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Subordonnés / {{ $ctx->getTypeLabel() }}</p>
                    <h1 class="mt-2 text-2xl font-black tracking-tight text-slate-950">Mes Chefs de Service</h1>
                    <p class="mt-1 text-sm text-slate-500">{{ $ctx->getNom() }} · {{ $chefsData->count() }} service(s)</p>
                </div>
                @if($evaluationsEnabled)
                    <a href="{{ route('directeur.evaluations.create') }}"
                       class="inline-flex items-center gap-2 rounded-2xl bg-indigo-600 px-4 py-2.5 text-xs font-black text-white shadow-sm transition hover:bg-indigo-700">
