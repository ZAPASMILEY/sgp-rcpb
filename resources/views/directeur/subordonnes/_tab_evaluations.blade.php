{{-- Tab : Évaluations des chefs de service du Directeur --}}
...
<form method="GET" action="{{ request()->url() }}"
    ...
    @if ($filters['statut'])
    <a href="{{ request()->url() }}?tab=evaluations{{ !empty($filters['serviceId']) ? '&service_id='.$filters['serviceId'] : '' }}"
        ...
        <table class="min-w-full text-sm">
            <thead>
                <tr class="border-b border-slate-200 bg-slate-50/80">
                    @if($showServiceCol)
                    <th class="px-5 py-3 text-left text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Service</th>
                    @endif
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($evaluations as $evaluation)
                    @php
                        ...
                        $serviceNom = $evaluation->evaluable?->nom ?? '-';
                    @endphp
                        ...
                        <td class="px-5 py-3.5 text-center">
                            <div class="inline-flex items-center gap-1">
                                ...
                                @if ($evaluation->statut !== 'brouillon')
                                    <a href="{{ route('directeur.evaluations.pdf', $evaluation) }}"
                                ...
                                @endif
                                @if ($evaluation->statut === 'brouillon')
                                    <form method="POST" action="{{ route('directeur.evaluations.submit', $evaluation) }}">
                                        @csrf @method('PATCH')
                                        <button type="submit"
                                                class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-slate-50 text-slate-400 transition hover:bg-blue-100 hover:text-blue-600"
                                                title="Soumettre">
                                ...
                                @if (in_array($evaluation->statut, ['brouillon', 'a_reviser']))
                                    <form method="POST" action="{{ route('directeur.evaluations.destroy', $evaluation) }}"
                    ...
                    <tr>
                        <td colspan="{{ $showServiceCol ? 5 : 4 }}" class="px-4 py-14 text-center">
                            <div class="mx-auto max-w-xs">
                                ...
                                <p class="mt-1 text-xs text-slate-400">Aucune évaluation enregistrée pour ce chef de service.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
