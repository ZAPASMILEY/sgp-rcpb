...
@section('title', 'Mes Chefs d\'Agence | '.config('app.name', 'SGP-RCPB'))
...
<div class="min-h-screen bg-[#f1f5f9] pb-10">
    <div class="w-full flex flex-col gap-6">
        ...
        {{-- Hero --}}
        <div class="relative overflow-hidden bg-gradient-to-br from-sky-700 via-sky-600 to-cyan-600 px-6 py-8 lg:px-10">
            <div class="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full bg-white/5 blur-3xl"></div>
            <div class="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <p class="text-[11px] font-black uppercase tracking-[0.25em] text-sky-200">Subordonnés · {{ $ctx->getTypeLabel() }}</p>
                    <h1 class="mt-1 text-2xl font-black tracking-tight text-white">Mes Chefs d'Agence</h1>
                    <p class="mt-1 text-sm text-sky-100/80">{{ $ctx->getNom() }} · {{ $agencesData->count() }} agence(s)</p>
                </div>
            </div>
        </div>
        ...
        {{-- KPIs --}}
        @php
            $totalEvals     = $agencesData->sum('evalCount');
            $totalObjectifs = $agencesData->sum('ficheCount');
            $totalAgents    = $agencesData->sum('agentsCount');
            $avecChef       = $agencesData->filter(fn($i) => $i['chef'] !== null)->count();
        @endphp
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            @foreach ([
                ['label' => 'Agences',        'value' => $agencesData->count(), 'icon' => 'fas fa-building',         'color' => 'bg-sky-50 border-sky-100',     'ic' => 'bg-sky-600 text-white'],
        ...
        {{-- Liste des chefs d'agence --}}
        <div class="admin-panel overflow-hidden">
            <div class="border-b border-slate-100 px-6 py-4 lg:px-8">
                <p class="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Responsables des agences</p>
            </div>
                    ...
                    @foreach ($agencesData as $item)
                        ...
                        <div class="flex flex-wrap items-center gap-5 px-6 py-5 transition hover:bg-slate-50/70 lg:px-8">
                            ...
                            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl
                                {{ $chef ? 'bg-sky-100 text-sky-700' : 'bg-slate-100 text-slate-400' }}
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <span class="inline-flex items-center gap-1 rounded-full border border-sky-200 bg-sky-50 px-2.5 py-0.5 text-[10px] font-black text-sky-700">
                                        <i class="fas fa-building text-[8px]"></i> {{ $agence->nom }}
                                    </span>
                                    @if ($chef?->role)
                            ...
                            <div class="flex shrink-0 items-center gap-2">
                                ...
                                @endif
                                <a href="{{ route('directeur.subordonnes.agence', $agence) }}"
