    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-indigo-700 via-indigo-600 to-blue-600 px-6 py-8 lg:px-10">
        ...
        <div class="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-black text-white leading-tight">{{ $agence->nom }}</h1>
                @if ($agence->chef)
                    <p class="mt-0.5 text-sm text-indigo-100/80">Chef d'agence : {{ $agence->chef->prenom }} {{ $agence->chef->nom }}</p>
                @endif
                @if ($agence->guichets->isNotEmpty())
                    <p class="text-xs text-indigo-200/70">{{ $agence->guichets->count() }} guichet(s)</p>
                @endif
            </div>
    ...
    <div class="w-full flex flex-col gap-5">
        ...
        <div class="rounded-[20px] border border-slate-100 bg-white px-6 py-6 shadow-sm">
            <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
                <div class="inline-flex gap-1 rounded-2xl border border-slate-200 bg-slate-100/70 p-1">
                    @foreach ([
                        ...
                        <a href="{{ route('directeur.subordonnes.agence', ['agence' => $agence->id, 'tab' => $t['key']]) }}"
                ...
                @if ($tab === 'evaluations')
                    @if ($evaluationsEnabled && $ficheAcceptee && !$evaluationEnCours)
                        <a href="{{ route('directeur.evaluations.create', ['agence_id' => $agence->id]) }}" class="ent-btn ent-btn-primary text-xs">
                ...
                @elseif ($tab === 'objectifs')
                    @if ($objectifsEnabled && !$ficheBlocksNew)
                        <a href="{{ route('directeur.subordonnes.agence.objectifs.create', $agence) }}" class="ent-btn ent-btn-primary text-xs">
                            <i class="fas fa-plus mr-1"></i> Assigner des objectifs
                        </a>
                    @else
                        <span title="{{ $ficheBlocksNew ? 'Une fiche d\'objectifs est déjà assignée à ce chef.' : ($objectifsDisabledMessage ?: 'Objectifs désactivés.') }}"
                              class="ent-btn-disabled-light text-xs">
                            <i class="fas fa-plus mr-1"></i> Assigner des objectifs
                        </span>
                    @endif
            ...
            @if ($tab === 'evaluations')
                ...
                {{-- Filtre --}}
                <form method="GET" action="{{ route('directeur.subordonnes.agence', $agence) }}"
                    ...
                    <a href="{{ route('directeur.subordonnes.agence', ['agence' => $agence->id, 'tab' => 'evaluations']) }}"
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @forelse ($evaluations as $eval)
                                    @php
                                        ...
                                        $cibleLabel = trim((string)($ident?->nom_prenom ?? '')) ?: ($chefAgenceUser?->name ?? $agence->nom);
                                    ...
                                    <tr class="align-top hover:bg-slate-50/60">
                                        ...
                                        <td class="px-4 py-4">
                                            ...
                                            <p class="mt-1 text-xs text-slate-400">Évalué</p>
                                        </td>
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <p class="font-semibold text-slate-700">{{ $anneeEval }} - Semestre {{ $semEval }}</p>
                                            <p class="mt-1 text-xs text-slate-400">Évaluateur : {{ $eval->evaluateur?->name ?? auth()->user()->name }}</p>
                                        </td>
                                        <td class="px-4 py-4">
            ...
            @else
                ...
                <form method="GET" action="{{ route('directeur.subordonnes.agence', $agence) }}"
                    ...
                    <a href="{{ route('directeur.subordonnes.agence', ['agence' => $agence->id, 'tab' => 'objectifs']) }}"
                       class="inline-flex items-center rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-600 transition hover:border-slate-300">
                        Effacer
                    </a>
                </form>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @forelse ($fiches as $fiche)
                                    ...
                                    <tr class="align-top hover:bg-slate-50/60">
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            <a href="{{ route('directeur.subordonnes.agence.objectifs.show', $fiche) }}"
                                               class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-slate-50 text-slate-500 transition hover:bg-blue-100 hover:text-blue-600"
