    ...
    <div class="w-full flex flex-col gap-6">
        ...
        <header class="admin-panel px-6 py-6 lg:px-8">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Espace Directeur / Subordonnés / {{ $agence->nom }}</p>
                    <h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{{ $agence->nom }}</h1>
                    @if ($agence->chef)
                        <p class="mt-2 text-sm text-slate-600">
                            Chef d'agence : <span class="font-semibold">{{ $agence->chef->prenom }} {{ $agence->chef->nom }}</span>
                        </p>
                    @endif
            ...
            @if ($tab === 'evaluations')
                ...
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center text-sm text-slate-400">
                        Aucune évaluation créée pour cette agence.
                    </div>
                @endforelse
                    ...
                    <div class="mb-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <div class="flex items-center gap-2">
                                <span class="inline-flex rounded-full border px-3 py-1 text-[11px] font-black {{ $sc }}">{{ $sl }}</span>
                                <a href="{{ route('directeur.subordonnes.agence.objectifs.show', $fiche) }}" class="ent-btn ent-btn-soft text-xs">Voir</a>
                            </div>
                        </div>
                    </div>
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center text-sm text-slate-400">
                        Aucune fiche d'objectifs assignée à cette agence.
                    </div>
                @endforelse
            @endif
        </div>
