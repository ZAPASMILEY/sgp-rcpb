            ...
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">
                        Espace Directeur / Subordonnés /
                        @if ($service) {{ $service->nom }}
                        @elseif ($secretaire) Secrétaire — {{ $secretaire->name }}
                        @endif
                    </p>
                ...
                @if ($service)
                    <a href="{{ route('directeur.subordonnes.service', ['service' => $service->id, 'tab' => 'objectifs']) }}" class="ent-btn ent-btn-soft">Retour</a>
                @elseif ($secretaire)
                    <a href="{{ route('directeur.subordonnes.secretaire', ['tab' => 'objectifs']) }}" class="ent-btn ent-btn-soft">Retour</a>
                @endif
            </div>
        </header>
        ...
        <section class="admin-panel px-6 py-6 lg:px-8">
            <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                    <p class="text-xs font-semibold uppercase tracking-[0.15em] text-slate-500">Assigné à</p>
                    <p class="mt-2 text-sm font-black text-slate-900">
                        @if ($service) {{ $service->nom }}
                        @elseif ($secretaire) {{ $secretaire->name }}
                        @endif
                    </p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
