...
<div class="min-h-screen bg-[#f1f5f9] pb-10">
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-violet-700 via-violet-600 to-purple-600 px-6 py-8 lg:px-10">
        ...
        <div class="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <p class="text-xs font-black uppercase tracking-[0.25em] text-violet-200">Espace Directeur · Subordonnés</p>
                <h1 class="mt-1 text-2xl font-black text-white leading-tight">Assigner une fiche d'objectifs</h1>
                <p class="mt-0.5 text-sm text-violet-100/80">Cible : <span class="font-semibold text-white">{{ $cibleLabel }}</span></p>
            </div>
            <a href="{{ $backRoute }}"
    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="overflow-hidden rounded-[24px] bg-white shadow-sm ring-1 ring-slate-100">
            ...
            <div class="border-b border-slate-100 px-6 py-4">
                ...
                <p class="mt-1 text-base font-black text-slate-900">{{ $cibleLabel }}</p>
                @if ($service)
                    @php $chef = trim(($service->chef_prenom ?? '').' '.($service->chef_nom ?? '')); @endphp
                    @if ($chef)
                        <p class="mt-0.5 text-xs text-slate-500">Chef de service : {{ $chef }}</p>
                    @endif
                @endif
                @if ($secretaire)
                    <p class="mt-0.5 text-xs text-slate-500">{{ $secretaire->email }}</p>
                @endif
            </div>

            <form method="POST" action="{{ route($storeRoute) }}" class="px-6 py-5 grid gap-5">
                @csrf
                @if ($hiddenField)
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="date_echeance" name="date_echeance" type="date"
                               ...
                               required
                               class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-violet-300 focus:ring-4 focus:ring-violet-100">
                    </div>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="titre_fiche" name="titre_fiche" value="{{ old('titre_fiche') }}" required
                           ...
                           class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-violet-300 focus:ring-4 focus:ring-violet-100">
                </div>
                ...
                <div class="space-y-2">
                        ...
                        @foreach ($oldObjectifs as $objectif)
                            <div class="objectif-row flex items-center gap-2">
                                <input type="text" name="objectifs[]" value="{{ $objectif }}" required
                                       ...
                                       class="flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-violet-300 focus:ring-4 focus:ring-violet-100">
                    ...
                    <button type="button" id="add-objectif-btn"
                            class="mt-1 inline-flex items-center gap-1.5 rounded-xl bg-violet-600 px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-violet-700">
                ...
                <div class="border-t border-slate-100 pt-4">
                    <button type="submit"
                            class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-violet-600 px-6 py-3.5 text-sm font-black text-white shadow-sm transition hover:bg-violet-700">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var inputClass = 'flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-violet-300 focus:ring-4 focus:ring-violet-100';
