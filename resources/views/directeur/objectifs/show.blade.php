...
@php
    ...
    $isOwnFiche    = true; // show() vérifie déjà l'appartenance
@endphp
<div class="min-h-screen bg-[#f1f5f9] pb-10">
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-emerald-700 via-emerald-600 to-teal-600 px-6 py-8 lg:px-10">
        ...
        <div class="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <p class="text-xs font-black uppercase tracking-[0.25em] text-emerald-200">Espace Directeur · Mes objectifs</p>
    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="flex flex-col gap-4 rounded-[24px] border-2 border-amber-200 bg-amber-50 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-3">
                <form action="{{ route('directeur.objectifs.statut', $fiche) }}" method="POST">
                    @csrf @method('PATCH')
                    ...
                    <div class="flex items-start gap-4 py-4 rounded-xl {{ $contested ? 'bg-rose-50 -mx-2 px-2' : '' }}">
                        ...
                        <div class="flex-1 min-w-0 pt-1">
                            ...
                            @else
                                ...
                                @if ($statut === 'acceptee')
                                    <form method="POST" action="{{ route('directeur.objectifs.lignes.avancement', [$fiche, $objectif]) }}" class="mt-2">
                                        @csrf @method('PATCH')
                        ...
                        @if ($statut !== 'acceptee' && ! $contested)
                            <form method="POST" action="{{ route('directeur.objectifs.lignes.contester', [$fiche, $objectif]) }}" class="shrink-0 pt-1">
                                @csrf @method('PATCH')
                                <button type="submit"
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-[24px] bg-white px-6 py-4 shadow-sm ring-1 ring-slate-100">
            ...
            <a href="{{ route('directeur.objectifs.pdf', $fiche) }}"
