    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div class="rounded-[20px] border border-slate-100 bg-white px-5 py-4 shadow-sm">
                ...
                @if ($statut === 'acceptee')
                    <form method="POST" action="{{ route('directeur.objectifs.avancement', $fiche) }}" class="mt-2">
                        @csrf @method('PATCH')
                        <select name="avancement_percentage" onchange="this.form.submit()"
                                class="w-full rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-700 outline-none focus:ring-1 focus:ring-emerald-400 cursor-pointer">
                            @for ($p = 0; $p <= 100; $p += 5)
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-[24px] bg-white px-6 py-4 shadow-sm ring-1 ring-slate-100">
            ...
            <a href="{{ route('directeur.objectifs.pdf', $fiche) }}"
               class="inline-flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm font-black text-emerald-700 shadow-sm transition hover:bg-emerald-100">
