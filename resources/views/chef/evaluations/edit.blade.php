{{--
    ──────────────────────────────────────────────────────────────────────────
    chef/evaluations/edit.blade.php — Modification d'un brouillon d'évaluation
    ──────────────────────────────────────────────────────────────────────────

    Variables reçues de ChefEvaluationController::edit() :
      $ctx                        — ChefEntity
      $evaluation                 — Evaluation (brouillon, avec criteres + identification)
      $agent                      — Agent évalué
      $ident                      — EvaluationIdentification
      $objectiveOptions           — Fiches d'objectifs actives de l'agent
      $existingSubjectiveCriteria — Critères subjectifs existants (format JS)
      $existingObjectiveCriteria  — Critères objectifs existants (format JS)
      $entiteNom                  — Nom de la structure du chef
      $displayYear                — Année d'affichage
      $openAnnee                  — Annee ouverte
      $openSemestres              — Collection des semestres ouverts
    ──────────────────────────────────────────────────────────────────────────
--}}
...
@php
    $currentSemestre = old('identification.semestre', (string) ($ident?->semestre ?? ''));
@endphp
    ...
    <div class="admin-shell min-h-screen px-4 py-6 sm:px-6 lg:px-10">
        <div class="w-full flex-col gap-6">
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-sm text-slate-600">
                            Évaluation de {{ trim($agent->prenom . ' ' . $agent->nom) }} — {{ $agent->role ?? 'Agent' }}
                        </p>
                    </div>
                    <a href="{{ route('chef.evaluations.show', $evaluation) }}" class="ent-btn ent-btn-soft">Retour</a>
                </div>
            </header>
            ...
            <section class="admin-panel px-6 py-6 lg:px-8">
                ...
                <form method="POST" action="{{ route('chef.evaluations.update', $evaluation) }}" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <section class="space-y-6">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500">
                                L'agent évalué est fixé. Vous pouvez ajuster les informations d'identification.
                            </p>
                        </div>

                        {{-- Agent évalué (lecture seule) --}}
                        <div class="rounded-2xl border border-cyan-100 bg-cyan-50/70 px-4 py-4">
                            ...
                            <p class="mt-1 text-sm text-slate-500">{{ $agent->role ?? 'Agent' }}</p>
                        </div>
                        ...
                        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <input id="identification_matricule" name="identification[matricule]" type="text"
                                       value="{{ old('identification.matricule', $ident?->matricule ?? $agent->matricule ?? '') }}"
                            ...
                            <div class="space-y-2">
                                ...
                                <input id="identification_emploi" name="identification[emploi]" type="text"
                                       value="{{ old('identification.emploi', $ident?->emploi ?? $agent->role ?? '') }}"
                                       class="ent-input">
                            </div>
                            <div class="space-y-2">
                                ...
                                <input id="identification_nom_prenom" name="identification[nom_prenom]" type="text"
                                       value="{{ old('identification.nom_prenom', $ident?->nom_prenom ?? trim($agent->prenom . ' ' . $agent->nom)) }}"
                                       class="ent-input">
                            </div>
                            <div class="space-y-2">
                                ...
                                <input id="identification_direction" name="identification[direction]" type="text"
                                       value="{{ old('identification.direction', $ident?->direction ?? $entiteNom) }}"
                                       class="ent-input">
                            </div>
                            <div class="space-y-2">
                                ...
                                <input id="identification_direction_service" name="identification[direction_service]" type="text"
                                       value="{{ old('identification.direction_service', $ident?->direction_service ?? $entiteNom) }}"
                                       class="ent-input">
                            </div>
                        </div>
                    ...
                    <section id="objective-section" class="space-y-5 border-t border-slate-200 pt-8">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-slate-500">
                                    Choisissez une fiche d'objectifs pour ajouter des critères, ou modifiez les existants. Barème : 1 à 5.
                                </p>
                            </div>
                        ...
                        <div class="grid gap-5 md:grid-cols-2">
                            <div class="space-y-2">
                                ...
                                <input id="signature_evalue_nom" name="signature_evalue_nom" type="text"
                                       ...
                                       class="ent-input" placeholder="Nom de l'agent évalué">
                    ...
                    <div class="flex flex-wrap items-center justify-end gap-3 border-t border-slate-200 pt-6">
                        <a href="{{ route('chef.evaluations.show', $evaluation) }}" class="ent-btn ent-btn-soft">Annuler</a>
...
@push('scripts')
    ...
    <script id="chef-edit-objective-options" type="application/json">@json($objectiveOptions ?? [])</script>
    <script id="chef-edit-subjective-data" type="application/json">@json(old('subjective_criteres') ? old('subjective_criteres') : $existingSubjectiveCriteria)</script>
    <script id="chef-edit-objective-data" type="application/json">@json(old('objective_criteres') ? old('objective_criteres') : $existingObjectiveCriteria)</script>
    <script id="chef-edit-formations-data" type="application/json">@json(old('identification.formations') ?? $ident?->formations ?? [])</script>
    <script id="chef-edit-experiences-data" type="application/json">@json(old('identification.experiences') ?? $ident?->experiences ?? [])</script>

    <script>
    document.addEventListener('DOMContentLoaded', function () {
        const objectiveOptions     = JSON.parse(document.getElementById('chef-edit-objective-options').textContent || '[]');
        const subjectiveData       = JSON.parse(document.getElementById('chef-edit-subjective-data').textContent || '[]');
        const objectiveData        = JSON.parse(document.getElementById('chef-edit-objective-data').textContent || '[]');
        const formationsData       = JSON.parse(document.getElementById('chef-edit-formations-data').textContent || '[]');
        const experiencesData      = JSON.parse(document.getElementById('chef-edit-experiences-data').textContent || '[]');
