{{--
    ──────────────────────────────────────────────────────────────────────────
    chef/evaluations/show.blade.php — Détail d'une évaluation
    ──────────────────────────────────────────────────────────────────────────

    Sert pour deux cas :
      • $isReceived = true → le chef est l'évalué (évaluation reçue d'un supérieur)
                             → boutons Accepter / Refuser si statut = soumis
      • $isCreated  = true → le chef est l'évaluateur (évaluation créée pour un agent)
                             → boutons Soumettre / Supprimer si statut = brouillon

    Variables reçues de ChefEvaluationController::show() :
      $ctx                — ChefEntity
      $evaluation         — Evaluation (avec criteres, identification chargés)
      $isReceived         — bool
      $isCreated          — bool
      $objectiveCriteria  — Collection des critères objectifs
      $subjectiveCriteria — Collection des critères subjectifs
      $note               — float (note finale)
      $mention            — string (Excellent|Bien|Passable|Insuffisant)
      $cibleLabel         — Nom de l'évalué
      $cibleType          — Rôle/type de l'évalué
      $statusClass        — Classes Tailwind du badge de statut
      $statusLabel        — Libellé du statut
      $ident              — EvaluationIdentification (ou null)
    ──────────────────────────────────────────────────────────────────────────
--}}
    ...
    <div class="w-full flex flex-col gap-6">
            ...
            <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
                ...
                <div>
                    ...
                    <p class="mt-2 text-2xl font-black text-emerald-700">
                        {{ number_format((float) $evaluation->note_finale, 2, ',', ' ') }}
                    </p>
        ...
        <section class="admin-panel px-6 py-6 lg:px-8">
            ...
            <div class="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                <div>
                    <span class="text-xs uppercase text-slate-500">Année</span>
                    <p class="mt-1 text-sm text-slate-800">
                        {{ $ident?->date_evaluation?->format('Y') ?? $evaluation->date_debut->format('Y') }}
                    </p>
                </div>
                <div>
                    <span class="text-xs uppercase text-slate-500">Nom et prénom</span>
                    <p class="mt-1 text-sm text-slate-800">{{ $ident?->nom_prenom ?? '-' }}</p>
                </div>
                <div>
                    <span class="text-xs uppercase text-slate-500">Semestre</span>
                    <p class="mt-1 text-sm text-slate-800">
                        {{ $ident?->semestre ? 'Semestre ' . $ident->semestre : '-' }}
                    </p>
                </div>
                <div>
                    <span class="text-xs uppercase text-slate-500">Date d'évaluation</span>
                    <p class="mt-1 text-sm text-slate-800">
                        {{ $ident?->date_evaluation?->format('d/m/Y') ?? '-' }}
                    </p>
                </div>
                <div>
                    <span class="text-xs uppercase text-slate-500">Emploi</span>
                    <p class="mt-1 text-sm text-slate-800">{{ $ident?->emploi ?? '-' }}</p>
                </div>
                <div>
                    <span class="text-xs uppercase text-slate-500">Matricule</span>
                    <p class="mt-1 text-sm text-slate-800">{{ $ident?->matricule ?? '-' }}</p>
                </div>
                <div>
                    <span class="text-xs uppercase text-slate-500">Entité</span>
                    <p class="mt-1 text-sm text-slate-800">{{ $ident?->direction ?? '-' }}</p>
                </div>
                <div>
                    <span class="text-xs uppercase text-slate-500">Structure</span>
                    <p class="mt-1 text-sm text-slate-800">{{ $ident?->direction_service ?? '-' }}</p>
                </div>
            </div>
                ...
                <div class="overflow-x-auto rounded-2xl border border-slate-200">
                    <table class="min-w-full text-left text-sm text-slate-700">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="px-3 py-3 text-slate-400">Aucune expérience renseignée.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                    ...
                    <article class="rounded-2xl border border-slate-200 bg-white p-5">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <span class="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">
                                Note {{ number_format((float) $criterion->note_globale, 2, ',', ' ') }}
                            </span>
                        </div>
                ...
                <div>
                    ...
                    @if ($evaluation->date_signature_evalue)
                        <p class="text-xs text-slate-400">
                            {{ \Carbon\Carbon::parse($evaluation->date_signature_evalue)->format('d/m/Y') }}
                        </p>
                    @endif
                </div>
                <div>
                    ...
                    @if ($evaluation->date_signature_evaluateur)
                        <p class="text-xs text-slate-400">
                            {{ \Carbon\Carbon::parse($evaluation->date_signature_evaluateur)->format('d/m/Y') }}
                        </p>
                    @endif
                </div>
            </div>
        ...
        <section class="admin-panel px-6 py-6 lg:px-8">
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    {{-- ── Évaluation REÇUE : Accepter / Refuser (si soumis) ── --}}
                    @if ($isReceived && $evaluation->statut === 'soumis')
                        <form method="POST"
                    ...
                    @if ($isCreated && $evaluation->statut === 'brouillon')
                        ...
                        <form method="POST"
                            ...
                            <button type="submit" class="ent-btn ent-btn-primary">
                                <i class="fas fa-paper-plane mr-2"></i>Soumettre à l'agent
                            </button>
                        </form>
                        <form method="POST"
                              action="{{ route('chef.evaluations.destroy', $evaluation) }}"
                              onsubmit="return confirm('Supprimer définitivement cette évaluation brouillon ?')">
                            @csrf
                            @method('DELETE')
                    ...
                    <a href="{{ route('chef.mon-espace') }}" class="ent-btn ent-btn-soft">
