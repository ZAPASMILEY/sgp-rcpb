    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-[#008751] via-[#006837] to-[#006837] px-6 py-8 lg:px-10">
        ...
        <div class="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p class="mt-0.5 text-sm text-blue-100/80">Année {{ $fiche->annee?->annee ?? $fiche->annee_id }}</p>
            </div>
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div class="rounded-[20px] border border-slate-100 bg-white px-5 py-4 shadow-sm">
                ...
                @if ($fiche->statut === 'acceptee')
                    <form method="POST" action="{{ route('chef.mes-fiches.avancement', $fiche) }}" class="mt-2">
                        @csrf @method('PATCH')
                        <select name="avancement_percentage" onchange="this.form.submit()"
                                class="w-full rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-700 outline-none focus:ring-1 focus:ring-blue-400 cursor-pointer">
                            @for ($p = 0; $p <= 100; $p += 5)
            ...
            <div class="divide-y divide-slate-50 px-6 py-2">
                @forelse ($fiche->objectifs as $index => $objectif)
                    <div class="flex items-start gap-4 py-4">
                        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-sm font-black text-blue-600 ring-1 ring-blue-100">
                            {{ $index + 1 }}
                        </div>
