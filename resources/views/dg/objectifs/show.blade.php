...
<div class="min-h-screen bg-[#f1f5f9] pb-10">
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-emerald-700 via-emerald-600 to-teal-600 px-6 py-8 lg:px-10">
        ...
        <div class="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <p class="text-xs font-black uppercase tracking-[0.25em] text-emerald-200">Espace DG · Mes objectifs</p>
                <h1 class="mt-1 text-2xl font-black text-white leading-tight">{{ $fiche->titre }}</h1>
                <p class="mt-0.5 text-sm text-emerald-100/80">Année {{ $fiche->annee?->annee ?? $fiche->annee_id }}</p>
            </div>
    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="flex flex-col gap-4 rounded-[24px] border-2 border-amber-200 bg-amber-50 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-3">
                <form action="{{ route('dg.objectifs.statut', $fiche) }}" method="POST">
                    @csrf @method('PATCH')
                ...
                <form action="{{ route('dg.objectifs.statut', $fiche) }}" method="POST">
                    ...
                    <button type="submit"
                            ...
                            class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-black text-white shadow-md shadow-emerald-200 transition hover:bg-emerald-700">
                ...
                @foreach ($fiche->objectifs as $index => $objectif)
                    ...
                    <div class="flex items-start gap-4 py-4 rounded-xl {{ $contested ? 'bg-rose-50 -mx-2 px-2' : '' }}">
                        ...
                        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-sm font-black ring-1
                            {{ $contested ? 'bg-rose-100 text-rose-600 ring-rose-200' : 'bg-emerald-50 text-emerald-600 ring-emerald-100' }}">
                            {{ $index + 1 }}
                        </div>
                        ...
                        <div class="flex-1 min-w-0 pt-1">
                            ...
                            @else
                                ...
                                @if ($statut === 'acceptee' && $isOwnFiche)
                                    <form method="POST" action="{{ route('dg.objectifs.lignes.avancement', [$fiche, $objectif]) }}" class="mt-2">
                                        @csrf @method('PATCH')
                                        <select name="avancement_percentage" onchange="this.form.submit()"
                                                class="w-full max-w-[160px] rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-700 outline-none focus:ring-1 focus:ring-emerald-400 cursor-pointer">
                                            @for ($p = 0; $p <= 100; $p += 5)
                        ...
                        @if ($statut !== 'acceptee' && ! $contested && $isOwnFiche)
                            <form method="POST" action="{{ route('dg.objectifs.lignes.contester', [$fiche, $objectif]) }}" class="shrink-0 pt-1">
                                @csrf @method('PATCH')
                                <button type="submit"
