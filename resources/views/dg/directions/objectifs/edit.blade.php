...
@php
    ...
    $heroDesc = match ($mode) {
        'contesté' => 'Les objectifs contestés (en orange) peuvent être modifiés. Les autres sont verrouillés.',
        'refusee'  => 'Le directeur a refusé cette fiche. Modifiez-la puis renvoyez-la.',
        default    => 'Modifiez la fiche avant de l\'envoyer au directeur.',
    };
    $heroIcon = match ($mode) {
        'contesté' => 'fa-flag',
...
<div class="min-h-screen bg-[#f1f5f9] pb-10">
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-violet-700 via-violet-600 to-purple-600 px-6 py-8 lg:px-10">
        ...
        <div class="pointer-events-none absolute -bottom-10 left-1/3 h-40 w-40 rounded-full bg-white/5 blur-2xl"></div>
            ...
            <div class="flex items-start gap-4">
                ...
                <div>
                    <div class="flex items-center gap-2 text-xs font-semibold text-white/60">
                        <a href="{{ route('dg.directions.show', ['direction' => $direction->id, 'tab' => 'objectifs']) }}" class="hover:text-white transition">{{ $direction->nom }}</a>
    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        @if ($isContested)
            <div class="flex items-start gap-4 rounded-[24px] border-2 border-orange-200 bg-orange-50 px-6 py-4">
                ...
                <div>
                    <p class="font-black text-orange-900">Objectifs contestés par le directeur</p>
                    <p class="mt-0.5 text-sm text-orange-700">
                        {{ $fiche->objectifs->where('statut', 'contesté')->count() }}
                        objectif(s) signalé(s) en orange. Corrigez-les puis renvoyez la fiche.
                    </p>
                </div>
            </div>
        @elseif ($isRefusee)
            <div class="flex items-start gap-4 rounded-[24px] border-2 border-rose-200 bg-rose-50 px-6 py-4">
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-sm text-rose-700">Le directeur a refusé cette fiche. Modifiez les objectifs et renvoyez-la.</p>
                </div>
            </div>
        @endif

        <form method="POST" action="{{ route('dg.directions.objectifs.update', $fiche) }}" class="flex flex-col gap-5">
            @csrf
            @method('PATCH')
                ...
                <div class="grid gap-5 px-6 py-5 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                            ...
                            <div>
                                <p class="text-sm font-black text-slate-800">{{ $directeurNom }}</p>
                                <p class="text-[10px] text-slate-400">Directeur — {{ $direction->nom }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Date assignation --}}
                    <div>
                        <label class="mb-1 block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Date d'assignation</label>
                    ...
                    {{-- Échéance --}}
                    <div>
                        <label class="mb-1 block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Échéance</label>
                        <div class="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                    ...
                    <div class="sm:col-span-2">
                        <label for="titre_fiche" class="mb-1 block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">
                            Titre de la fiche @if (!$isContested)<span class="text-rose-500">*</span>@endif
                        </label>
                        @if ($isContested)
                            <div class="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                        ...
                        @else
                            <input type="text" id="titre_fiche" name="titre_fiche"
                                   ...
                                   placeholder="Ex : Contrat d'objectifs — {{ $direction->nom }}"
            ...
            <div class="overflow-hidden rounded-[24px] bg-white shadow-sm ring-1 ring-slate-100">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-black text-slate-800">
                                ...
                                <span class="ml-1.5 rounded-full bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-500">{{ $fiche->objectifs->count() }}</span>
                            </p>
                            @if ($isContested)
                ...
                <div id="objectifs-container" class="divide-y divide-slate-50 px-6 py-4 space-y-2">
                    @foreach ($fiche->objectifs as $index => $objectif)
                        @php $contested = ($objectif->statut ?? '') === 'contesté'; @endphp
                        ...
                        @elseif ($isContested && $contested)
                            {{-- Ligne contestée — éditable ou supprimable --}}
                        ...
                        @else
                            {{-- Brouillon / refusée : pleine édition --}}
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        @if ($isContested)
                            ...
                            <button type="submit" name="action" value="renvoyer"
                                    class="inline-flex shrink-0 items-center gap-2 rounded-xl bg-orange-600 px-6 py-2.5 text-sm font-black text-white shadow-md transition hover:bg-orange-700">
                                <i class="fas fa-paper-plane text-xs"></i> Enregistrer et renvoyer au directeur
                            </button>
                        @elseif ($isRefusee)
                            <button type="submit" name="action" value="renvoyer"
                                    class="inline-flex shrink-0 items-center gap-2 rounded-xl bg-rose-600 px-6 py-2.5 text-sm font-black text-white shadow-md transition hover:bg-rose-700">
                                <i class="fas fa-paper-plane text-xs"></i> Corriger et renvoyer au directeur
                            </button>
                        @else
                            <button type="submit" name="action" value="brouillon"
                            ...
                            <button type="submit" name="action" value="renvoyer"
                                    class="inline-flex shrink-0 items-center gap-2 rounded-xl bg-violet-600 px-6 py-2.5 text-sm font-black text-white shadow-md transition hover:bg-violet-700">
