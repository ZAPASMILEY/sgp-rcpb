...
@section('title', 'Comparaison inter-période · DG · ' . config('app.name', 'SGP-RCPB'))
    ...
    <div class="border-b border-slate-200 bg-white px-6 py-5 lg:px-10">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-600 text-white shadow">
                    <i class="fas fa-code-compare text-sm"></i>
                </div>
                <div>
                    <p class="text-[10px] font-black uppercase tracking-widest text-indigo-500">Espace DG · Pilotage</p>
                    <h1 class="text-lg font-black text-slate-900">Comparaison inter-période</h1>
                </div>
            </div>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('dg.comparaison.index', array_merge(request()->query(), ['format' => 'pdf'])) }}"
                   target="_blank"
        ...
        {{-- ── Sélecteurs ──────────────────────────────────────────────────── --}}
        <form method="GET" action="{{ route('dg.comparaison.index') }}"
            ...
            <div class="flex items-end pb-2 text-slate-400">
    ...
    <div class="px-4 pt-6 lg:px-8 flex flex-col gap-6">
        ...
        <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            <div class="border-b border-slate-100 px-6 py-4">
                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Détail des indicateurs</p>
                ...
                <tbody>
                    @foreach ($sections as $sectionTitle => $rows)
                        <tr class="bg-slate-50/80 border-y border-slate-100">
                            <td colspan="4" class="px-6 py-2 text-[10px] font-black uppercase tracking-widest text-slate-500">{{ $sectionTitle }}</td>
                        </tr>
                        @foreach ($rows as $row)
                        @php
                            $v1   = $stats1[$row['key']] ?? 0;
                            $v2   = $stats2[$row['key']] ?? 0;
                            $inv  = $row['inv']; $dec = $row['dec'];
                            $up   = $v2 > $v1; $dn = $v2 < $v1;
                            $good = $inv ? $dn : $up;
                            $bad  = $inv ? $up : $dn;
                            $diff = $v2 - $v1;
                        ...
                        <tr class="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="px-2 py-3 text-center">
                                ...
                                <span class="text-xs font-black {{ $good ? 'text-emerald-600' : 'text-rose-600' }}">
                                    <i class="fas {{ $up ? 'fa-arrow-up' : 'fa-arrow-down' }} text-[8px]"></i> {{ $df }}
                                </span>
                                @else
        ...
        <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            <div class="border-b border-slate-100 px-6 py-4">
                ...
                <h2 class="mt-0.5 text-base font-black text-slate-900">
                    Distribution des appréciations ·
                    <span class="text-blue-700">{{ $a1 }}</span>
                    <span class="mx-1 font-normal text-slate-300">vs</span>
                    <span class="text-orange-600">{{ $a2 }}</span>
                </h2>
            </div>
            <div class="grid gap-6 px-6 py-5 lg:grid-cols-[1fr_300px]">
                <div class="relative h-56 lg:h-auto">
                    <canvas id="compChart"></canvas>
                </div>
                <div class="flex flex-col gap-2">
                    @foreach ([
                        ['key' => 'excellent',   'label' => 'Excellent',   'icon' => 'fa-star',               'bg' => 'bg-emerald-50', 'border' => 'border-emerald-100', 'ic' => 'text-emerald-500', 'tx' => 'text-emerald-800', 'inv' => false],
                        ['key' => 'bien',        'label' => 'Bien',        'icon' => 'fa-thumbs-up',          'bg' => 'bg-sky-50',     'border' => 'border-sky-100',     'ic' => 'text-sky-500',     'tx' => 'text-sky-800',     'inv' => false],
                        ['key' => 'passable',    'label' => 'Passable',    'icon' => 'fa-circle-half-stroke', 'bg' => 'bg-amber-50',   'border' => 'border-amber-100',   'ic' => 'text-amber-500',   'tx' => 'text-amber-800',   'inv' => false],
                        ['key' => 'insuffisant', 'label' => 'Insuffisant', 'icon' => 'fa-triangle-exclamation','bg'=> 'bg-rose-50',    'border' => 'border-rose-100',    'ic' => 'text-rose-500',    'tx' => 'text-rose-800',    'inv' => true],
                    ] as $app)
                    @php $v1a = $stats1[$app['key']]; $v2a = $stats2[$app['key']]; $da = $v2a - $v1a; $isG = $app['inv'] ? $da < 0 : $da > 0; @endphp
                    <div class="flex items-center gap-2 rounded-xl border {{ $app['border'] }} {{ $app['bg'] }} px-3 py-2.5">
                        <i class="fas {{ $app['icon'] }} w-4 text-center {{ $app['ic'] }} text-sm"></i>
                        <span class="flex-1 text-sm font-bold {{ $app['tx'] }}">{{ $app['label'] }}</span>
                        <span class="w-7 text-right text-sm font-black text-blue-700">{{ $v1a }}</span>
                        <i class="fas fa-arrow-right text-[9px] text-slate-300"></i>
                        <span class="w-7 text-sm font-black text-orange-600">{{ $v2a }}</span>
                        @if($da != 0)
                        <span class="w-8 text-right text-xs font-black {{ $isG ? 'text-emerald-600' : 'text-rose-600' }}">{{ $da > 0 ? '+' : '' }}{{ $da }}</span>
                        @else <span class="w-8"></span>
                        @endif
                    </div>
                    @endforeach

                    <div class="mt-1 flex items-center gap-4 rounded-xl bg-slate-50 px-4 py-2.5 text-xs text-slate-500">
            ...
            <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-blue-100">
                ...
                <div class="px-6 py-5 space-y-4">
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        <div class="rounded-xl bg-slate-50 px-3 py-2.5">
                            <p class="text-[10px] font-bold uppercase tracking-wide text-slate-400">Sur total</p>
...
(function () {
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            plugins: { legend: { position: 'top', labels: { font: { family:'Inter', weight:'700', size:11 }, padding:16 } } },
            scales: {
