    ...
    <div class="w-full flex flex-col gap-6">
        ...
        <header class="admin-panel px-6 py-6 lg:px-8">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <p class="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Espace DG / Délégations Techniques</p>
                    <h1 class="mt-1 text-2xl font-black tracking-tight text-slate-950">{{ $delegation->region }}</h1>
                    <p class="mt-1 text-sm text-slate-500">{{ $delegation->ville }} — {{ $delegation->caisses->count() }} caisse(s) · {{ $delegation->agences->count() }} agence(s)</p>
                </div>
                <div class="flex shrink-0 flex-wrap items-center gap-2">
                    <a href="{{ route('dg.delegations.pdf', array_merge(['delegation' => $delegation->id], request()->query())) }}"
                       class="ent-btn ent-btn-soft">
                        <i class="fas fa-file-pdf mr-2"></i>Télécharger PDF
                    </a>
                    <a href="{{ route('dg.delegations') }}" class="ent-btn ent-btn-soft">
        ...
        <section class="admin-panel px-6 py-6 lg:px-8">
            <h2 class="text-sm font-black uppercase tracking-widest text-slate-700 mb-4">Personnel de direction</h2>
        ...
        <section class="admin-panel overflow-hidden">
            <div class="border-b border-slate-100 px-6 py-4 flex items-center justify-between">
                <h2 class="text-sm font-black uppercase tracking-widest text-slate-700">
                    Caisses
                    <span class="ml-2 rounded-full bg-amber-100 px-2 py-0.5 text-xs font-black text-amber-700">{{ $delegation->caisses->count() }}</span>
                </h2>
            </div>
            @if ($delegation->caisses->isEmpty())
                <p class="px-6 py-8 text-sm text-slate-400">Aucune caisse rattachée.</p>
            @else
        ...
        {{-- Séparateur titre personnel --}}
        <div class="flex items-center gap-3">
            <div class="h-px flex-1 bg-slate-200"></div>
            <span class="text-xs font-black uppercase tracking-widest text-slate-400">Personnel évalué de la délégation</span>
            <div class="h-px flex-1 bg-slate-200"></div>
        </div>

        @include('dg.reseau._personnel_panel', [
            'filterRoute'  => 'dg.delegations.show',
            'filterParams' => ['delegation' => $delegation->id],
        ])
