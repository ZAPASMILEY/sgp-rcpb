    ...
    <div class="w-full flex flex-col gap-6">
        ...
        <header class="admin-panel px-6 py-6 lg:px-8">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Mon Espace / Directeur Général</p>
                    <h1 class="mt-2 text-3xl font-black tracking-tight text-slate-900">{{ $user->name }}</h1>
                    <p class="mt-1 text-sm text-slate-500">Directeur Général</p>
                </div>
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            @php
            $quickCards = [
                ['label'=>'Évaluations',  'value'=>$evaluationsStats['total'],  'icon'=>'fas fa-star',        'tone'=>'border-emerald-100 bg-emerald-50/80 text-emerald-900',   'iconWrap'=>'bg-white text-emerald-600'],
                ['label'=>'Acceptées',    'value'=>$evaluationsStats['valide'], 'icon'=>'fas fa-check',       'tone'=>'border-teal-100 bg-teal-50/80 text-teal-900',            'iconWrap'=>'bg-white text-teal-600'],
                ['label'=>'Objectifs',    'value'=>$fichesStats['total'],       'icon'=>'fas fa-bullseye',    'tone'=>'border-emerald-100 bg-emerald-50/80 text-emerald-900',   'iconWrap'=>'bg-white text-emerald-500'],
                ['label'=>'Acceptés',     'value'=>$fichesStats['acceptees'],   'icon'=>'fas fa-circle-check','tone'=>'border-slate-100 bg-white text-slate-900',               'iconWrap'=>'bg-slate-100 text-slate-600'],
            ];
            @endphp
        ...
        <div class="admin-panel px-6 py-6">
            ...
            <div class="mb-6 flex flex-wrap items-center gap-4">
                <div class="inline-flex gap-1 rounded-2xl border border-slate-200 bg-slate-100/70 p-1">
                    <a href="{{ route('dg.mon-espace') }}?tab=evaluations"
                       class="inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold transition
                        ...
                        <span class="rounded-full px-2 py-0.5 text-[11px] font-black
                            {{ $tab === 'evaluations' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600' }}">
                            {{ $evaluationsStats['total'] }}
                        </span>
                    </a>
                    <a href="{{ route('dg.mon-espace') }}?tab=objectifs"
                       class="inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold transition
                           {{ $tab === 'objectifs'
                               ? 'border border-slate-200 bg-white text-emerald-700 shadow-sm'
                               : 'text-slate-500 hover:text-slate-800' }}">
            ...
            @if ($tab === 'evaluations')
                ...
                {{-- Filtre --}}
                <form method="GET" action="{{ route('dg.mon-espace') }}"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="statut"
                            ...
                            <option value="valide"  @selected($filters['statut'] === 'valide')>Acceptée</option>
                                ...
                                @forelse ($evaluations as $evaluation)
                                    ...
                                    <tr class="align-top hover:bg-slate-50/60">
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            <a href="{{ route('dg.evaluations.show', $evaluation) }}"
                                    ...
                                    <tr>
                                        <td colspan="7" class="px-4 py-12 text-center">
                                            <div class="mx-auto max-w-sm rounded-[24px] border border-dashed border-slate-200 bg-slate-50 px-6 py-8">
                                                ...
                                                <p class="mt-1 text-xs text-slate-500">Vous n'avez pas encore d'évaluation reçue du PCA.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
                ...
                <form method="GET" action="{{ route('dg.mon-espace') }}"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="statut"
                            ...
                            <option value="acceptee"   @selected($filters['statut'] === 'acceptee')>Accepté</option>
                            <option value="refusee"    @selected($filters['statut'] === 'refusee')>Refusé</option>
                        </select>
                    </div>
                    <button type="submit"
                            class="inline-flex items-center rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @forelse ($fiches as $fiche)
                                    ...
                                    <tr class="hover:bg-slate-50/60">
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            <a href="{{ route('dg.objectifs.show', $fiche) }}"
                                               class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-slate-50 text-slate-500 transition hover:bg-emerald-100 hover:text-emerald-600"
                                               title="Voir la fiche">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                    ...
                                    <tr>
                                        <td colspan="7" class="px-4 py-12 text-center">
                                            <div class="mx-auto max-w-sm rounded-[24px] border border-dashed border-slate-200 bg-slate-50 px-6 py-8">
                                                ...
                                                <p class="mt-1 text-xs text-slate-500">Vous n'avez pas encore de fiche d'objectifs assignée par le PCA.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
