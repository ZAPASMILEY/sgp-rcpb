...
<div class="min-h-screen bg-[#f1f5f9] px-4 pb-8 pt-4 lg:px-8">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                @if ($evaluation->statut === 'soumis')
                    ...
                    <button type="button"
                            onclick="document.getElementById('modalRefusDg').classList.remove('hidden');document.getElementById('modalRefusDg').classList.add('flex')"
    ...
    <section class="flex flex-col gap-4 rounded-[24px] border-2 border-rose-200 bg-rose-50 px-6 py-5">
        ...
        <form action="{{ route('dg.evaluations.reclamer', $evaluation) }}" method="POST" class="mt-2 flex flex-col gap-3">
            @csrf
...
{{-- Modal Refus DG --}}
<div id="modalRefusDg" class="fixed inset-0 z-50 hidden items-center justify-center">
    ...
    <div class="relative w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl mx-4">
        ...
        <form action="{{ route('dg.evaluations.statut', $evaluation) }}" method="POST" class="mt-5 flex flex-col gap-4">
            @csrf @method('PATCH')
            <input type="hidden" name="action" value="refuser">
            <div class="flex flex-col gap-1.5">
                <label for="motif_refus_dg" class="text-sm font-semibold text-slate-700">Motif du refus</label>
                <textarea id="motif_refus_dg" name="motif_refus" rows="4" required maxlength="1000"
            ...
            <div class="flex justify-end gap-3">
                <button type="button"
                        onclick="document.getElementById('modalRefusDg').classList.add('hidden');document.getElementById('modalRefusDg').classList.remove('flex')"
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        document.getElementById('modalRefusDg').classList.add('hidden');
        document.getElementById('modalRefusDg').classList.remove('flex');
    }
});
</script>
@endsection
