...
{{--
    Dropdown téléporté au niveau du <body> via JS pour échapper à tout contexte
    d'empilement (stacking context) créé par backdrop-filter, filter, transform,
    etc. dans les layouts ancêtres.
--}}
...
<script>
(function () {
    const bellId   = @json($bellId);
    const btn      = document.getElementById('notif-bell-btn-'    + bellId);
    const dropdown = document.getElementById('notif-dropdown-'    + bellId);

    if (!btn || !dropdown) return;

    // ── Téléporter le dropdown au niveau du <body> ──────────────────────────
    // Cela lui permet d'ignorer tous les stacking contexts des ancêtres
    // (backdrop-filter, filter, transform…) et d'apparaître au-dessus de tout.
    document.body.appendChild(dropdown);

    // ── Positionner le dropdown sous le bouton cloche ────────────────────────
    function reposition() {
