...
<body class="h-full antialiased">
    ...
    <nav class="sidebar shadow" id="sidebar">
        ...
        <div class="mt-auto border-t border-white/10 p-3">
            <div class="sidebar-user-compact flex items-center gap-3 rounded-xl bg-white/10 px-3 py-3">
                <div class="user-avatar flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white text-xs font-black text-indigo-700">
                    {{ strtoupper(substr($user?->name ?? 'U', 0, 1)) }}
                </div>
                <div class="sidebar-user-info min-w-0 flex-1">
                    <p class="truncate text-sm font-bold text-white">{{ $user?->name ?? 'Collaborateur' }}</p>
                    <p class="text-[10px] font-semibold uppercase tracking-wider text-white/60">Session active</p>
                </div>
                <form action="{{ route('subordonne.logout') }}" method="POST" class="sidebar-user-info">
                    @csrf
    ...
    <div class="main-content">
        <header class="flex h-12 shrink-0 items-center justify-between border-b border-slate-100 bg-white/80 px-4 backdrop-blur-sm">
            ...
            @include('layouts._notif_bell', ['bellId' => 'sub'])
        </header>
    ...
    <script>
        ...
        const STORAGE_KEY = 'subordonne-sidebar-collapsed';
