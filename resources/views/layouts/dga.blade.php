...
<body class="h-full antialiased bg-slate-50">
    <div class="flex min-h-screen">
        ...
        {{-- Sidebar --}}
        <nav id="sidebar"
             style="width:260px; background:linear-gradient(180deg,#7c3aed 0%,#5b21b6 100%); color:#fff; display:flex; flex-direction:column;"
             class="shadow">
            <div class="p-6 text-center border-b border-white/10">
                ...
                <p class="mt-1 text-[11px] font-semibold uppercase tracking-widest text-white/60">Directeur General Adjoint</p>
            </div>
                    ...
                    @foreach($section['items'] as $item)
                        ...
                        <a href="{{ $link }}"
                           class="flex items-center px-6 py-2.5 my-0.5 rounded-lg mx-3 transition text-sm
                               {{ $isActive ? 'bg-white text-violet-800 font-bold shadow' : 'text-white/80 hover:bg-white/10' }}">
            ...
            <div class="px-6 pb-6 pt-4 border-t border-white/10">
                <form method="POST" action="{{ route('dga.logout') }}">
                    @csrf
                    <button type="submit"
