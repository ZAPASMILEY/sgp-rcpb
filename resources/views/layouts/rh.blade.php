...
<head>
    ...
    <title>@yield('title', 'Espace RH')</title>
    ...
    <style>
        ...
        .admin-panel { background: #fff; border-radius: 1.5rem; border: 1px solid #e2e8f0; box-shadow: 0 1px 4px rgba(0,0,0,0.04); }
...
<body class="h-full antialiased">
    ...
    <nav class="sidebar shadow" id="sidebar">
        ...
        <div class="sidebar-header">
            <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-full border-2 border-white/20 bg-white/10 text-purple-200 shadow">
                <i class="fas fa-user-tie text-2xl"></i>
            </div>
            <h5 class="mt-3 text-lg font-black text-white">Espace RH</h5>
            <p class="mt-0.5 text-[11px] font-semibold uppercase tracking-widest text-white/60">Ressources Humaines</p>
            @if($user)
                ...
                @foreach($section['items'] as $item)
                    @php
                        ...
                        $isActive = request()->routeIs($item['route'].'*') &&
                                    ($query ? str_contains(request()->getQueryString() ?? '', explode('=', $query)[1]) : !request()->has('tab'));
        ...
        <div class="mt-auto border-t border-white/10 p-3">
            <div class="sidebar-user-compact flex items-center gap-3 rounded-xl bg-white/10 px-3 py-3">
                <div class="user-avatar flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white text-xs font-black text-purple-700">
                    {{ strtoupper(substr($user?->name ?? 'R', 0, 1)) }}
                </div>
                <div class="sidebar-user-info min-w-0 flex-1">
                    <p class="truncate text-sm font-bold text-white">{{ $user?->name ?? 'RH' }}</p>
                    <p class="text-[10px] font-semibold uppercase tracking-wider text-white/60">Ressources Humaines</p>
                </div>
                <form action="{{ route('rh.logout') }}" method="POST" class="sidebar-user-info">
                    @csrf
    ...
    <div class="main-content">
        <header class="flex h-12 shrink-0 items-center justify-between border-b border-slate-100 bg-white/80 px-4 backdrop-blur-sm">
            ...
            <span class="hidden text-sm font-black text-slate-400 lg:block">Espace Ressources Humaines</span>
            @include('layouts._notif_bell', ['bellId' => 'rh'])
        </header>
