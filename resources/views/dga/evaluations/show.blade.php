    ...
    <div class="w-full flex-col gap-6">
        ...
        <div class="flex flex-col gap-4 rounded-[24px] border-2 border-amber-200 bg-amber-50 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-3">
                <button type="button"
                        onclick="document.getElementById('modalRefusDga').classList.remove('hidden');document.getElementById('modalRefusDga').classList.add('flex')"
        ...
        <div class="flex flex-col gap-4 rounded-[24px] border-2 border-rose-200 bg-rose-50 px-6 py-5">
            ...
            <form action="{{ route('dga.evaluations.reclamer', $evaluation) }}" method="POST" class="mt-2 flex flex-col gap-3">
                @csrf
                <label class="text-sm font-semibold text-rose-800">Soumettre une réclamation</label>
                <textarea name="reclamation" rows="3"
                    class="w-full rounded-xl border border-rose-200 bg-white px-4 py-2.5 text-sm text-slate-800 shadow-sm focus:border-rose-400 focus:outline-none"
...
{{-- Modal Refus DGA --}}
<div id="modalRefusDga" class="fixed inset-0 z-50 hidden items-center justify-center">
    ...
    <div class="relative w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl mx-4">
        ...
        <form action="{{ route('dga.evaluations.statut', $evaluation) }}" method="POST" class="mt-5 flex flex-col gap-4">
            @csrf @method('PATCH')
            <input type="hidden" name="action" value="refuser">
            <div class="flex flex-col gap-1.5">
                <label for="motif_refus_dga" class="text-sm font-semibold text-slate-700">Motif du refus</label>
                <textarea id="motif_refus_dga" name="motif_refus" rows="4" required maxlength="1000"
            ...
            <div class="flex justify-end gap-3">
                <button type="button"
                        onclick="document.getElementById('modalRefusDga').classList.add('hidden');document.getElementById('modalRefusDga').classList.remove('flex')"
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        document.getElementById('modalRefusDga').classList.add('hidden');
        document.getElementById('modalRefusDga').classList.remove('flex');
    }
});
</script>
@endsection
