                ...
                @foreach ($fiche->objectifs as $index => $objectif)
                    ...
                    <div class="flex items-start gap-4 py-4 rounded-xl {{ $contested ? 'bg-rose-50 -mx-2 px-2' : '' }}">
                        ...
                        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-sm font-black ring-1
                            {{ $contested ? 'bg-rose-100 text-rose-600 ring-rose-200' : 'bg-indigo-50 text-indigo-600 ring-indigo-100' }}">
                            {{ $index + 1 }}
                        </div>
                        ...
                        <div class="flex-1 min-w-0 pt-1">
                            ...
                            @else
                                ...
                                @if ($statut === 'acceptee' && $isOwnFiche)
                                    <form method="POST" action="{{ route('dga.objectifs.lignes.avancement', [$fiche, $objectif]) }}" class="mt-2">
                                        @csrf @method('PATCH')
                        ...
                        @if ($statut !== 'acceptee' && ! $contested && $isOwnFiche)
                            <form method="POST" action="{{ route('dga.objectifs.lignes.contester', [$fiche, $objectif]) }}" class="shrink-0 pt-1">
                                @csrf @method('PATCH')
                                <button type="submit"
