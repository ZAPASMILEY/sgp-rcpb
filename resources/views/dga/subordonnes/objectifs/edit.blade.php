...
@php
    ...
    $heroStyle = match ($mode) {
        ...
        default    => 'background: linear-gradient(135deg, #3730a3, #4338ca, #4f46e5)',
    };
    ...
    $backUrl = route('dga.sub-objectifs.show', $fiche);
@endphp
<div class="min-h-screen bg-[#f1f5f9] pb-10">
            ...
            <div class="flex items-start gap-4">
                ...
                <div>
                    <div class="flex items-center gap-2 text-xs font-semibold text-white/60">
                        <a href="{{ $backUrl }}" class="hover:text-white transition">Fiche</a>
    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="flex items-start gap-4 rounded-[24px] border-2 border-orange-200 bg-orange-50 px-6 py-4">
            ...
            <div>
                <p class="font-black text-orange-900">Objectifs contestés par {{ $subordonne?->name ?? 'le subordonné' }}</p>
        ...
        <form method="POST" action="{{ route('dga.sub-objectifs.update', $fiche) }}" class="flex flex-col gap-5">
            @csrf
            @method('PUT')
                ...
                <div class="grid gap-5 px-6 py-5 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        ...
                        <div class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                            ...
                            <div>
                                <p class="text-sm font-black text-slate-800">{{ $subordonne?->name ?? '—' }}</p>
                                <p class="text-[10px] text-slate-400">Subordonné du DGA</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        @else
                            <input type="text" id="titre_fiche" name="titre_fiche"
                                   ...
                                   class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 outline-none transition placeholder:text-slate-300 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100">
                        @endif
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 px-6 py-4">
                    <div class="flex items-center gap-3">
                        <span class="flex h-8 w-8 items-center justify-center rounded-xl
                            {{ $isContested ? 'bg-orange-100 text-orange-600' : 'bg-indigo-100 text-indigo-600' }}">
                    ...
                    @if (!$isContested)
                        <button type="button" id="add-objectif-btn"
                                class="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-4 py-2 text-xs font-black text-white shadow-sm transition hover:bg-indigo-700">
                ...
                <div id="objectifs-container" class="divide-y divide-slate-50 px-6 py-4 space-y-2">
                    ...
                    @else
                        <div class="objectif-row flex items-center gap-3 py-2">
                            <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-indigo-50 text-[11px] font-black text-indigo-600">
                                {{ $index + 1 }}
                            </div>
                            <input type="text" name="objectifs[]" required
                                   value="{{ old('objectifs.'.$loop->index, $objectif->description) }}"
                                   placeholder="Description de l'objectif"
                                   class="flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 outline-none transition placeholder:text-slate-300 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var inputClass  = 'flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 outline-none transition placeholder:text-slate-300 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100';
    ...
    function makeRow() {
        ...
        row.innerHTML =
            '<div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-indigo-50 text-[11px] font-black text-indigo-600">' + idx + '</div>'
