    ...
    <div class="w-full flex flex-col gap-6">
        ...
        <header class="admin-panel px-6 py-6 lg:px-8">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Espace DGA / Subordonnés</p>
                    <h1 class="mt-1 text-2xl font-black tracking-tight text-slate-950">Assigner une fiche d'objectifs</h1>
                    <p class="mt-1 text-sm text-slate-500">Créez une fiche et rattachez-la directement au collaborateur concerné.</p>
                </div>
        ...
        <section class="admin-panel px-6 py-6 lg:px-8">
            @if ($subordonnes->isEmpty())
                <div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 py-8 text-center">
                    ...
                    <p class="mt-2 text-sm font-black text-slate-700">Aucun subordonné disponible</p>
                    <p class="mt-1 text-xs text-slate-500">Aucun Directeur Technique ou secrétaire configuré.</p>
                </div>
            @else
                <form method="POST" action="{{ route('dga.sub-objectifs.store') }}" class="grid gap-5">
                    @csrf

                    {{-- Cible --}}
                    @if ($lockSubordonne && $resolvedSubordonne)
                        <div class="rounded-2xl border border-violet-100 bg-violet-50/70 px-4 py-4">
                            <p class="text-xs font-black uppercase tracking-[0.16em] text-violet-700">Collaborateur cible</p>
                            <p class="mt-2 text-base font-black text-slate-900">{{ $resolvedSubordonne['nom'] }}</p>
                            <p class="mt-1 text-sm text-slate-500">
                                {{ $resolvedSubordonne['role_label'] ?? '' }} — La fiche sera assignée automatiquement à ce collaborateur.
                            </p>
                    ...
                    @else
                        <div class="space-y-2">
                            ...
                            <select id="subordonne_id" name="subordonne_id" class="ent-input" required>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="space-y-2">
                            ...
                            <input type="date" value="{{ now()->toDateString() }}" readonly class="ent-input bg-slate-100 text-slate-600">
                        </div>
                        <div class="space-y-2">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" id="titre_fiche" name="titre_fiche" value="{{ old('titre_fiche') }}"
                               required class="ent-input"
                               placeholder="Ex : Contrat d'objectifs 2026">
                    </div>
                    ...
                    <button type="submit" class="ent-btn ent-btn-primary justify-center px-5 py-3 text-sm mt-2">
                        <i class="fas fa-paper-plane mr-2"></i>Assigner la fiche
                    </button>
                </form>
            @endif
        </section>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    btn.addEventListener('click', function () {
        ...
        row.innerHTML =
            ...
            '<button type="button" class="remove-objectif-btn flex h-9 w-9 items-center justify-center rounded-xl bg-rose-50 text-rose-400 transition hover:bg-rose-100 hover:text-rose-600"><i class="fas fa-trash text-xs"></i></button>';
