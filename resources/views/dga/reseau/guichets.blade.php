    ...
    <div class="w-full flex flex-col gap-6">
        ...
        <form method="GET" class="admin-panel px-5 py-4">
            <div class="flex flex-wrap items-end gap-3">
                ...
                @if($search || $agenceId)
                    <a href="{{ route('dga.reseau.guichets') }}" class="ent-btn ent-btn-soft">Réinitialiser</a>
                @endif
            </div>
        </form>
                            ...
                            @foreach($guichets as $guichet)
                                <tr class="hover:bg-slate-50/60 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-slate-600">
                                        {{ $guichet->chef?->numero_telephone ?? '—' }}
                                    </td>
                                    <td class="px-4 py-3 text-center">
                                        @php $gn = $guichetNotes[$guichet->id] ?? ['moyenne' => null, 'total' => 0]; @endphp
                                        @if($gn['moyenne'] !== null)
                                            @php $c = $gn['moyenne'] >= 8.5 ? 'bg-emerald-100 text-emerald-700' : ($gn['moyenne'] >= 7 ? 'bg-blue-100 text-blue-700' : ($gn['moyenne'] >= 5 ? 'bg-amber-100 text-amber-700' : 'bg-rose-100 text-rose-700')); @endphp
                                            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-bold {{ $c }}">{{ number_format($gn['moyenne'], 2) }}</span>
                                        @else
