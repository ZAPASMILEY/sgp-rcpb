...
@php
    ...
    $sc = match($statut) {
        'acceptee'  => ['label'=>'Acceptée',   'bg'=>'bg-slate-100',  'text'=>'text-slate-700',  'dot'=>'bg-slate-500',  'border'=>'border-slate-200'],
    ...
    $isOwnFiche    = $fiche->assignable_type === \App\Models\User::class && (int) $fiche->assignable_id === auth()->id();
@endphp
<div class="min-h-screen bg-[#f1f5f9] pb-10">

    {{-- Hero --}}
    <div class="relative overflow-hidden bg-gradient-to-br from-slate-700 via-slate-600 to-slate-500 px-6 py-8 lg:px-10">
        <div class="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full bg-white/5 blur-3xl"></div>
        <div class="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <p class="text-xs font-black uppercase tracking-[0.25em] text-slate-300">Mon Espace · Mes objectifs</p>
                <h1 class="mt-1 text-2xl font-black text-white leading-tight">{{ $fiche->titre }}</h1>
                <p class="mt-0.5 text-sm text-slate-200/80">Année {{ $fiche->annee?->annee ?? $fiche->annee_id }}</p>
            </div>
    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="flex flex-col gap-4 rounded-[24px] border-2 border-amber-200 bg-amber-50 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-3">
                <form action="{{ route('subordonne.objectifs.statut', $fiche) }}" method="POST">
                    @csrf @method('PATCH')
                ...
                @foreach ($fiche->objectifs as $index => $objectif)
                    ...
                    <div class="flex items-start gap-4 py-4 rounded-xl {{ $contested ? 'bg-rose-50 -mx-2 px-2' : '' }}">
                        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-sm font-black ring-1
                            {{ $contested ? 'bg-rose-100 text-rose-600 ring-rose-200' : 'bg-slate-50 text-slate-600 ring-slate-200' }}">
                            {{ $index + 1 }}
                        </div>
                        ...
                        <div class="flex-1 min-w-0 pt-1">
                            ...
                            @else
                                ...
                                @if ($statut === 'acceptee' && $isOwnFiche)
                                    <form method="POST" action="{{ route('subordonne.objectifs.lignes.avancement', [$fiche, $objectif]) }}" class="mt-2">
                                        @csrf @method('PATCH')
                                        <select name="avancement_percentage" onchange="this.form.submit()"
                                                class="w-full max-w-[160px] rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-700 outline-none focus:ring-1 focus:ring-slate-400 cursor-pointer">
                                            @for ($p = 0; $p <= 100; $p += 5)
                        ...
                        @if ($statut !== 'acceptee' && ! $contested && $isOwnFiche)
                            <form method="POST" action="{{ route('subordonne.objectifs.lignes.contester', [$fiche, $objectif]) }}" class="shrink-0 pt-1">
                                @csrf @method('PATCH')
                                <button type="submit"
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-[24px] bg-white px-6 py-4 shadow-sm ring-1 ring-slate-100">
            ...
            <a href="{{ route('subordonne.objectifs.pdf', $fiche) }}"
               class="inline-flex items-center gap-2 rounded-xl bg-slate-700 px-4 py-2.5 text-sm font-black text-white shadow-sm transition hover:bg-slate-800">
