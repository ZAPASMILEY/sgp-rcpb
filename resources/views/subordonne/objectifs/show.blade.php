    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div class="rounded-[20px] border border-slate-100 bg-white px-5 py-4 shadow-sm">
                ...
                @if ($statut === 'acceptee')
                    <form method="POST" action="{{ route('subordonne.objectifs.avancement', $fiche) }}" class="mt-2">
                        @csrf @method('PATCH')
                        <select name="avancement_percentage" onchange="this.form.submit()"
                                class="w-full rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-700 outline-none focus:ring-1 focus:ring-slate-400 cursor-pointer">
                            @for ($p = 0; $p <= 100; $p += 5)
            ...
            <div class="divide-y divide-slate-50 px-6 py-2">
                @forelse ($fiche->objectifs as $index => $objectif)
                    <div class="flex items-start gap-4 py-4">
                        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-slate-50 text-sm font-black text-slate-600 ring-1 ring-slate-200">
                            {{ $index + 1 }}
                        </div>
