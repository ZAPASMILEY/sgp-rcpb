    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
            ...
            <div class="border-t border-slate-100 px-5 py-3 flex items-center justify-between">
                <span class="text-xs text-slate-400">{{ $alertes->total() }} alerte{{ $alertes->total() > 1 ? 's' : '' }}</span>
                {{ $alertes->links() }}
            </div>
            @endif
        </div>
...
{{-- Modal Créer Alerte --}}
<div id="modal-alerte-create" class="fixed inset-0 z-50 hidden">
    <div class="flex min-h-screen items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <form method="POST" action="{{ route('gerer.alertes.store') }}">
                @csrf
                <div class="space-y-4">
                    <div>
