    ...
    <main class="admin-shell min-h-screen px-4 py-6 sm:px-6 lg:px-10">
        <div class="mx-auto max-w-3xl">
            <section class="admin-panel p-6 sm:p-8">
                ...
                <form method="POST" action="{{ route('admin.directions.update', $direction) }}" class="mt-6 grid gap-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="entite_id" name="entite_id" required class="ent-select">
                            ...
                            @foreach ($entites as $entite)
                                <option value="{{ $entite->id }}" @selected((string) old('entite_id', $direction->entite_id) === (string) $entite->id)>{{ $entite->nom }}</option>
                            @endforeach
                        </select>
                    </div>
