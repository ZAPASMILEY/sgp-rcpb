        ...
        <form method="POST" action="{{ route('admin.entites.directions.store') }}" target="_top" class="p-8 lg:p-12 space-y-8">
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-2">
                    ...
                    <input id="nom-input" name="nom" type="text" value="{{ old('nom') }}" required
                    ...
                    <div id="dga-warning" class="hidden flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs text-amber-700 mt-2">
                        <i class="fas fa-exclamation-triangle mt-0.5 shrink-0 text-amber-500"></i>
                        <span>La Direction Générale Adjointe dispose d'un formulaire dédié.
                            <a href="{{ route('admin.direction-dga.configurer') }}" class="font-bold underline">Utiliser le formulaire DGA</a>
                        </span>
                    </div>
                </div>
            </div>
                    ...
                    @if($directeurs->isEmpty())
                        @if(($totalDirecteurs ?? 0) === 0)
                            <div class="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs text-amber-700">
                                <i class="fas fa-exclamation-triangle mt-0.5 shrink-0 text-amber-500"></i>
                                <span>Aucun agent avec le rôle <strong>Directeur de Direction</strong> n'est enregistré. <a href="{{ route('admin.agents.create') }}" class="font-bold underline">Créer un agent</a></span>
                            </div>
                        @else
                            <div class="flex items-start gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs text-rose-700">
                                ...
                                <span>Tous les agents <strong>Directeur de Direction</strong> ({{ $totalDirecteurs }}) sont déjà affectés à une direction. <a href="{{ route('admin.agents.create') }}" class="font-bold underline">Ajouter un nouvel agent</a></span>
                            </div>
                        @endif
                    @endif
                ...
                <div class="space-y-2">
                    ...
                    @if($secretaires->isEmpty())
                        @if(($totalSecretaires ?? 0) === 0)
                            <div class="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs text-amber-700">
                                <i class="fas fa-exclamation-triangle mt-0.5 shrink-0 text-amber-500"></i>
                                <span>Aucun agent avec le rôle <strong>Secrétaire de Direction</strong> n'est enregistré. <a href="{{ route('admin.agents.create') }}" class="font-bold underline">Créer un agent</a></span>
                            </div>
                        @else
                            <div class="flex items-start gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs text-rose-700">
                                <i class="fas fa-ban mt-0.5 shrink-0 text-rose-400"></i>
                                <span>Toutes les agents <strong>Secrétaire de Direction</strong> ({{ $totalSecretaires }}) sont déjà affectées à une direction. <a href="{{ route('admin.agents.create') }}" class="font-bold underline">Ajouter un nouvel agent</a></span>
                            </div>
                        @endif
                    @endif
...
<script>
    ...
    const dgaWarning = document.getElementById('dga-warning');

    function checkDgaName() {
        const val = nomInput.value.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        const isDga = val.includes('adjointe') || val.includes('dga');
        dgaWarning.classList.toggle('hidden', !isDga);
        dgaWarning.classList.toggle('flex', isDga);
    }

    nomInput.addEventListener('input', checkDgaName);
    checkDgaName();
