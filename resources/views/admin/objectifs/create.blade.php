            ...
            <section class="admin-panel ent-window p-6 sm:p-8">
                ...
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-sm text-slate-600">Assignez un objectif a une entite, direction, service ou agent.</p>
                    </div>
                ...
                <form method="POST" action="{{ route('admin.objectifs.store') }}" class="mt-6 grid gap-5">
                    @csrf
                    ...
                    <div class="ent-form-grid">
                        ...
                        <div class="space-y-2">
                            ...
                            <input id="date_echeance" name="date_echeance" type="date" value="{{ old('date_echeance') }}" required class="ent-input">
                        </div>
                    </div>
                    ...
                    <div class="ent-form-grid">
                        <div class="space-y-2">
                            ...
                            <select id="assignable_type" name="assignable_type" required class="ent-select">
                                ...
                                <option value="entite" @selected(old('assignable_type') === 'entite')>Entite</option>
                                <option value="direction" @selected(old('assignable_type') === 'direction')>Direction</option>
                                <option value="service" @selected(old('assignable_type') === 'service')>Service</option>
                                <option value="agent" @selected(old('assignable_type') === 'agent')>Agent</option>
                            </select>
                        </div>
                        <div class="space-y-2">
                            <label for="assignable_id" class="text-sm font-semibold text-slate-700">Cible</label>
                            <select id="assignable_id" name="assignable_id" required class="ent-select" data-previous-target="{{ old('assignable_id') }}">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="commentaire" name="commentaire" rows="6" required class="ent-input">{{ old('commentaire') }}</textarea>
                    </div>
