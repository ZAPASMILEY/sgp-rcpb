...
<div class="min-h-screen bg-[#f1f5f9] pb-10">
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-slate-800 via-slate-700 to-slate-600 px-6 py-8 lg:px-10">
        ...
        <div class="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <p class="text-xs font-black uppercase tracking-[0.25em] text-slate-300">Administration · Création</p>
                <h1 class="mt-1 text-2xl font-black text-white leading-tight">Nouvel objectif</h1>
                <p class="mt-0.5 text-sm text-slate-300/80">Assignez un objectif à une entité, direction, service ou agent.</p>
            </div>
    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="overflow-hidden rounded-[24px] bg-white shadow-sm ring-1 ring-slate-100">
            <div class="border-b border-slate-100 px-6 py-4">
                <p class="text-sm font-black text-slate-800">Informations de l'objectif</p>
                <p class="mt-0.5 text-xs text-slate-500">Remplissez tous les champs pour créer un nouvel objectif.</p>
            </div>

            <form method="POST" action="{{ route('admin.objectifs.store') }}" class="px-6 py-5 grid gap-5">
                @csrf

                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="space-y-1.5">
                        <label for="date" class="text-xs font-black uppercase tracking-[0.14em] text-slate-500">Date</label>
                        <input id="date" type="date" value="{{ $today }}" readonly
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="date_echeance" name="date_echeance" type="date" value="{{ old('date_echeance') }}" required
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="assignable_id" name="assignable_id" required
                                data-previous-target="{{ old('assignable_id') }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea id="commentaire" name="commentaire" rows="6" required
                              class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-slate-300 focus:ring-4 focus:ring-slate-100 resize-none">{{ old('commentaire') }}</textarea>
                </div>
                ...
                <div class="border-t border-slate-100 pt-4">
                    <button type="submit"
                        ...
                        <i class="fas fa-plus text-xs"></i> Créer l'objectif
                    </button>
                </div>
            </form>
        </div>
