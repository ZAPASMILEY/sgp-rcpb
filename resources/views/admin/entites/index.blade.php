...
<div id="faitiere-agent-form" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-[28px] border border-white/70 bg-white p-6 shadow-2xl lg:p-8">
        ...
        <div class="mb-6">
            <p class="text-xs font-black uppercase tracking-[0.25em] text-orange-500">Affectation</p>
            <h2 class="mt-2 text-2xl font-black tracking-tight text-slate-900">Affecter un agent à un service</h2>
            <p class="mt-1 text-sm text-slate-500">Sélectionnez un agent existant et le service auquel l'affecter.</p>
        </div>

        <form method="POST" action="{{ route('admin.entites.agents.store') }}" class="space-y-5">
            @csrf

            <div>
                <label class="mb-1 block text-xs font-bold uppercase tracking-wider text-slate-500">Agent <span class="text-rose-500">*</span></label>
                @if(($agents_disponibles ?? collect())->isEmpty())
                    <div class="mb-2 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs text-amber-700">
                        <i class="fas fa-exclamation-triangle mt-0.5 shrink-0 text-amber-500"></i>
                        <span>Aucun agent sans service disponible pour l'affectation. <a href="{{ route('admin.agents.create') }}" class="font-bold underline">Créer un agent</a></span>
                    </div>
                @endif
            ...
            <div>
                ...
                <select name="service_id" required class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 shadow-sm focus:border-orange-400 focus:ring-orange-400">
                    ...
                    @foreach ($allServices as $svc)
                        <option value="{{ $svc->id }}">{{ $svc->nom }}{{ $svc->direction ? ' ('.$svc->direction->nom.')' : '' }}</option>
                    @endforeach
                </select>
            </div>
