...
<div class="min-h-screen bg-[#f8fafc] p-4 lg:p-8 font-sans">
    <div class="mb-4">
        <a href="{{ url()->previous() }}" class="inline-flex items-center gap-2 text-emerald-600 hover:text-emerald-800 font-semibold text-sm">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
                ...
                <div class="flex items-center gap-2 mt-1">
                    ...
                    <span class="text-xs font-bold text-emerald-500 uppercase tracking-widest">Unités Centrales</span>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('admin.entites.directions.create') }}"
                   class="h-12 px-6 bg-emerald-500 text-white rounded-2xl text-xs font-black uppercase tracking-widest flex items-center gap-2 hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-100">
            ...
            <table class="w-full border-separate border-spacing-y-3">
                ...
                <tbody>
                    ...
                    @php
                        ...
                        $directeurEmail = $directeur->email ?? '';
                    @endphp
                    <tr class="bg-white group hover:scale-[1.01] transition-all duration-200"
                        data-search-content="{{ strtolower(trim($direction->nom . ' ' . $directeurNom . ' ' . $directeurEmail . ' ' . ($direction->services_count ?? 0) . ' services')) }}">

                        {{-- Ordre --}}
                        <td class="px-6 py-5 first:rounded-l-[24px] border-y border-l border-slate-50 shadow-sm shadow-slate-200/20">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-slate-50 text-slate-400 font-black text-xs group-hover:bg-emerald-500 group-hover:text-white transition-colors">
                        ...
                        <td class="px-6 py-5 border-y border-slate-50 shadow-sm shadow-slate-200/20">
                            <div class="flex items-center gap-4">
                                <div class="h-12 w-12 bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-2xl flex items-center justify-center text-emerald-600 font-black text-lg shadow-sm border border-white">
                                    {{ substr($direction->nom, 0, 1) }}
                                </div>
                                <div class="min-w-0">
                                    <p class="font-black text-slate-800 text-base leading-tight truncate group-hover:text-emerald-600 transition-colors">{{ $direction->nom }}</p>
                                    <p class="text-[10px] text-slate-300 font-bold uppercase mt-1 tracking-widest">ID: #{{ $direction->id }}</p>
                                </div>
                            </div>
                        </td>

                        {{-- Responsable — utilise la relation Agent (directeur_agent_id → agents) --}}
                        <td class="px-6 py-5 border-y border-slate-50 shadow-sm shadow-slate-200/20">
                            @if ($directeur)
                                <p class="font-bold text-slate-700 text-sm">{{ $directeurNom }}</p>
                                @if ($directeurEmail)
                                    <p class="text-[11px] text-emerald-500 font-medium lowercase flex items-center gap-1 mt-1">
                                        <i class="far fa-envelope opacity-50"></i> {{ $directeurEmail }}
                                    </p>
                                @endif
                            @else
                                <p class="text-[11px] text-slate-300 italic">Non assigné</p>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-5 border-y border-slate-50 shadow-sm shadow-slate-200/20 text-center">
                            <a href="{{ route('admin.services.index', ['direction_id' => $direction->id, 'source' => 'faitiere']) }}"
                               class="px-4 py-2 bg-slate-50 text-slate-500 text-[10px] font-black uppercase rounded-xl border border-slate-100 hover:bg-emerald-500 hover:text-white hover:border-emerald-500 transition-all">
                        ...
                        <td class="px-6 py-5 last:rounded-r-[24px] border-y border-r border-slate-50 shadow-sm shadow-slate-200/20 text-right">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('admin.directions.show', $direction) }}"
                                   class="h-9 w-9 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-400 hover:text-emerald-500 hover:border-emerald-200 transition-all shadow-sm"
                                   title="Voir">
                                    <i class="far fa-eye text-sm"></i>
                                </a>
                                <a href="{{ route('admin.directions.edit', $direction) }}"
                                   class="h-9 w-9 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-400 hover:text-blue-500 hover:border-blue-200 transition-all shadow-sm"
                                   title="Modifier">
                                    <i class="far fa-edit text-sm"></i>
                                </a>
                                <form method="POST" action="{{ route('admin.directions.destroy', $direction) }}" class="inline"
                                      onsubmit="return confirm('Confirmer la suppression de « {{ addslashes($direction->nom) }} » ?');">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                            class="h-9 w-9 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-400 hover:text-rose-500 hover:border-rose-200 transition-all shadow-sm"
                                            title="Supprimer">
                    ...
                    <tr>
                        <td colspan="5" class="py-24 text-center">
                            <div class="flex flex-col items-center">
                                <div class="h-20 w-20 bg-emerald-50 rounded-full flex items-center justify-center text-emerald-200 text-3xl mb-4">
                                    <i class="fas fa-folder-open"></i>
                                </div>
                                <p class="text-slate-400 font-bold italic">Aucune direction trouvée.</p>
                                <a href="{{ route('admin.entites.directions.create') }}"
                                   class="mt-4 text-emerald-500 text-xs font-black uppercase hover:underline">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        rows.forEach(function (row) {
            ...
            [cells[1], cells[2], cells[3]].forEach(function (cell) {
                ...
                if (txt.length >= 2 && txt !== '-' && txt !== 'n/a' && txt !== 'Non assigné') pool.add(txt);
            });
        });
