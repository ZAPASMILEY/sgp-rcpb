...
@section('title', 'Agents du guichet | '.config('app.name', 'SGP-RCPB'))

@section('content')
    <div class="mb-4">
        <a href="{{ route('admin.guichets.index') }}" class="inline-flex items-center gap-2 text-cyan-600 hover:text-cyan-800 font-semibold text-sm">
            <i class="fas fa-arrow-left"></i>
            <span>Retour aux guichets</span>
        </a>
    </div>
        ...
        <div class="w-full space-y-6">
            <section class="admin-panel p-6">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Guichets / Agents</p>
                        <h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{{ $guichet->nom }}</h1>
                        <p class="mt-2 text-sm text-slate-600">
                            @if ($guichet->agence)
                                Agence : {{ $guichet->agence->nom }}
                                @if ($guichet->agence->delegationTechnique)
                                    · {{ $guichet->agence->delegationTechnique->region }}
                                @endif
                            @endif
                            @if ($guichet->chef)
                                · Chef : {{ $guichet->chef->prenom }} {{ $guichet->chef->nom }}
                            @endif
                        </p>
                    </div>
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('admin.guichets.index') }}" class="ent-btn ent-btn-soft">Index guichets</a>
                        <a href="{{ route('admin.guichets.agents.create', $guichet) }}" class="ent-btn ent-btn-primary">Ajouter un agent</a>
                    </div>
                </div>
                <div class="mt-4">
                    <div class="inline-flex px-4 py-3 rounded-2xl bg-slate-50 border border-slate-100 text-xs font-black uppercase tracking-widest text-slate-400">{{ $agents->count() }} agent(s)</div>
                </div>
            </section>
                                ...
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap text-right">
                                        <div class="ent-actions inline-flex items-center gap-2">
                                            <a href="{{ route('admin.agents.show', $agent) }}" class="ent-btn ent-btn-soft inline-flex h-7 w-7 items-center justify-center p-0" title="Voir l'agent">
                                            ...
                                            <a href="{{ route('admin.agents.edit', $agent) }}" class="ent-btn ent-btn-soft inline-flex h-7 w-7 items-center justify-center p-0" title="Modifier l'agent">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="h-3.5 w-3.5">
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="py-10 text-center text-sm text-slate-500">
                                        Aucun agent affecté à ce guichet.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
