            ...
            <section class="admin-panel ent-window h-full w-full p-6 sm:p-8">
                ...
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        ...
                        <h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Nouvelle caisse</h1>
                        <p class="mt-2 text-sm text-slate-600">Renseignez les coordonnees du directeur de caisse, le secretariat et le superviseur technique.</p>
                    </div>
                ...
                @if ($directions->isEmpty())
                    <div class="mt-6 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                        Ajoutez d'abord un Directeur Technique pour pouvoir choisir un superviseur.
                    </div>
                @endif

                <form method="POST" action="{{ route('admin.caisses.store') }}" target="_top" class="mt-6 grid gap-6">
                    @csrf
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-xs text-slate-500">Le superviseur sera filtre selon cette delegation.</p>
                    </div>

                    <div class="space-y-2">
                        <label for="nom" class="text-sm font-semibold text-slate-700">Nom de la caisse</label>
                        <input id="nom" name="nom" type="text" value="{{ old('nom') }}" required class="ent-input" placeholder="Ex: Caisse de Ouagadougou Centre">
                    </div>
                    ...
                    <div class="ent-card space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <input id="directeur_nom" name="directeur_nom" type="text" value="{{ old('directeur_nom') }}" required class="ent-input" placeholder="Nom et prenom du directeur de caisse">
                        </div>
                        <div class="ent-form-grid">
                            <div class="space-y-2">
                                <label for="directeur_email" class="text-sm font-semibold text-slate-700">Email</label>
                                <input id="directeur_email" name="directeur_email" type="email" value="{{ old('directeur_email') }}" required class="ent-input" placeholder="directeur.caisse@rcpb.org">
                            </div>
                            <div class="space-y-2">
                                <label for="directeur_telephone" class="text-sm font-semibold text-slate-700">Telephone</label>
                                <input id="directeur_telephone" name="directeur_telephone" type="text" value="{{ old('directeur_telephone') }}" required class="ent-input" placeholder="+226 70 00 00 00">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="ent-form-grid">
                        <div class="space-y-2">
                            ...
                            <input id="secretariat_telephone" name="secretariat_telephone" type="text" value="{{ old('secretariat_telephone') }}" required class="ent-input" placeholder="+226 25 00 00 00">
                        </div>
                        <div class="space-y-2">
                            <label for="superviseur_direction_id" class="text-sm font-semibold text-slate-700">Directeur technique superviseur</label>
                            <select id="superviseur_direction_id" name="superviseur_direction_id" required class="ent-select" @disabled($directions->isEmpty() || $delegations->isEmpty())>
                    ...
                    <button type="submit" @disabled($directions->isEmpty() || $delegations->isEmpty()) class="ent-btn ent-btn-primary justify-center px-5 py-3 text-sm disabled:cursor-not-allowed disabled:opacity-60">
