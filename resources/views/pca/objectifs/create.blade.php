...
<div class="min-h-screen bg-[#f1f5f9] pb-10">
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-emerald-700 via-emerald-600 to-teal-600 px-6 py-8 lg:px-10">
        ...
        <div class="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <p class="text-xs font-black uppercase tracking-[0.25em] text-emerald-200">Objectifs · Création</p>
                <h1 class="mt-1 text-2xl font-black text-white leading-tight">Nouvel objectif</h1>
                <p class="mt-0.5 text-sm text-emerald-100/80">Créez une fiche d'objectifs destinée au Directeur Général.</p>
            </div>
    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="overflow-hidden rounded-[24px] bg-white shadow-sm ring-1 ring-slate-100">
            <div class="border-b border-slate-100 px-6 py-4">
                <p class="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Cible unique</p>
                <p class="mt-1 text-base font-black text-slate-900">{{ $dgUser?->name ?? 'DG non configuré' }}</p>
                <p class="mt-0.5 text-xs text-slate-500">Les objectifs créés par le PCA sont assignés uniquement au Directeur Général.</p>
            </div>

            <form method="POST" action="{{ route('pca.objectifs.store') }}" class="px-6 py-5 grid gap-5">
                @csrf
