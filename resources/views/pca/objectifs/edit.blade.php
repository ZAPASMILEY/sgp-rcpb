...
@section('content')
@php
    $mode = $fiche->statut ?? 'brouillon'; // 'brouillon' | 'contesté' | 'refusee'
    $isContested = $mode === 'contesté';
    $isRefusee   = $mode === 'refusee';

    // Inline styles pour les gradients (classes -700/-600 absentes du build Tailwind JIT)
    $heroStyle = match ($mode) {
        ...
        default    => 'background: linear-gradient(135deg, #b45309, #d97706, #ea580c)',
    };
    ...
    $heroDesc = match ($mode) {
        ...
        'refusee'  => 'Le Directeur Général a refusé cette fiche. Modifiez-la puis renvoyez-la.',
        default    => 'Modifiez la fiche avant de l\'envoyer au Directeur Général.',
    };
    $heroIcon = match ($mode) {
        'contesté' => 'fa-flag',
...
<div class="min-h-screen bg-[#f1f5f9] pb-10">
    ...
    <div class="px-4 pt-6 lg:px-8">
        ...
        <div class="flex items-start gap-4 rounded-[24px] border-2 border-orange-200 bg-orange-50 px-6 py-4">
            ...
            <div>
                <p class="font-black text-orange-900">Objectifs contestés par le DG</p>
        ...
        <div class="flex items-start gap-4 rounded-[24px] border-2 border-rose-200 bg-rose-50 px-6 py-4">
            ...
            <div>
                ...
                <p class="mt-0.5 text-sm text-rose-700">Le Directeur Général a refusé cette fiche. Modifiez les objectifs et renvoyez-la.</p>
            </div>
        </div>
        @endif

        <form method="POST" action="{{ route('pca.objectifs.update', $fiche) }}" class="flex flex-col gap-5">
            @csrf
            @method('PUT')
                ...
                <div class="grid gap-5 px-6 py-5 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                            ...
                            <div>
                                <p class="text-sm font-black text-slate-800">{{ $dgUser?->name ?? 'Directeur Général' }}</p>
                                <p class="text-[10px] text-slate-400">Les objectifs du PCA sont assignés uniquement au Directeur Général</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                            ...
                            <span class="text-sm font-semibold text-slate-500">
                                {{ \Carbon\Carbon::parse($fiche->date)->translatedFormat('d/m/Y') }}
                            </span>
                        </div>
                    </div>
                    ...
                    {{-- Année --}}
                    <div>
                        <label class="mb-1 block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Année</label>
                        <div class="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                            <i class="fas fa-calendar text-xs text-slate-400"></i>
                            <span class="text-sm font-semibold text-slate-500">
                                {{ $fiche->annee_value ?? now()->year }}
                            </span>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 px-6 py-4">
                    <div class="flex items-center gap-3">
                        <span class="flex h-8 w-8 items-center justify-center rounded-xl
                            {{ $isContested ? 'bg-orange-100 text-orange-600' : 'bg-emerald-100 text-emerald-600' }}">
                            <i class="fas fa-list-check text-sm"></i>
                        </span>
                        <div>
                            <p class="text-sm font-black text-slate-800">
                                Objectifs à assigner
                    ...
                    @if (!$isContested)
                        <button type="button" id="add-objectif-btn"
                                class="inline-flex items-center gap-1.5 rounded-xl bg-emerald-600 px-4 py-2 text-xs font-black text-white shadow-sm transition hover:bg-emerald-700">
                ...
                <div id="objectifs-container" class="divide-y divide-slate-50 px-6 py-4 space-y-2">
                    ...
                    @else
                        ...
                        <div class="objectif-row flex items-center gap-3 py-2">
                            <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-[11px] font-black text-emerald-600">
                                {{ $index + 1 }}
                            </div>
                            <input type="text" name="objectifs[]" required
                                   value="{{ old('objectifs.'.$loop->index, $objectif->description) }}"
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        @if ($isContested)
                            ...
                            <button type="submit" name="action" value="renvoyer"
                                ...
                                <i class="fas fa-paper-plane text-xs"></i> Enregistrer et renvoyer au DG
                            </button>
                        @elseif ($isRefusee)
                            <button type="submit" name="action" value="renvoyer"
                                    class="inline-flex shrink-0 items-center gap-2 rounded-xl px-6 py-2.5 text-sm font-black text-white shadow-md transition"
                                    style="background:#e11d48" onmouseover="this.style.background='#be123c'" onmouseout="this.style.background='#e11d48'">
                                <i class="fas fa-paper-plane text-xs"></i> Corriger et renvoyer au DG
                            </button>
                        @else
                            <button type="submit" name="action" value="brouillon"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function makeRow() {
        ...
        row.innerHTML =
            '<div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-[11px] font-black text-emerald-600">' + idx + '</div>'
